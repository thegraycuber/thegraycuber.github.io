function core_color_custom(plt){
	...
	return gradient([palette.mono,palette.bright,palette.red],1);
...
function gradient(gradSteps,repeats){
	...
	for (var g = 0; g < gradSteps.length; g++){
		...
		for (var jump = 0; jump < jumps; jump++){
			grad_output.push(lerpColor(gradSteps[g],gradSteps[gn],jump/jumps));
		}
		// var gn = (g + 1) % gradSteps.length;
...
function colorFactor(){
	...
	return gradientFactor([palette.mono,palette.bright,palette.red],1);

	// var color_output = [];
	
	// if (schemeId == 'Rainbow'){color_output = gradientFactor(120,390,70,0,100);
	// } else if (schemeId == 'Fire'){color_output = gradientFactor(405,330,100,75,0);
	// } else if (schemeId == 'Ice'){color_output = gradientFactor(210,330,50,90,20);
	// } else if (schemeId == 'Sunset'){ color_output = gradientFactor(270,450,70,10,100);
	// } else if (schemeId == 'Pastel'){color_output = gradientFactor(30,210,60,95,30);
	// } else if (schemeId == 'Cotton Candy'){ color_output = gradientFactor(330,180,90,90,50);
	// } else if (schemeId == 'Dark Mode'){ color_output = gradientFactor(210,330,50,0,85);
	// } else if (schemeId == 'Forest'){ color_output = gradientFactor(150,30,70,10,100);
	
	// }
	// return color_output;
}
function gradientFactor(gradSteps,repeats){
	var grad_output = [];
	var start_len = gradSteps.length;
	for (var rep = 1; rep < repeats; rep++){
		for (var step = 0; step < start_len; step++){
	...
	for (var g = 0; g < gradSteps.length; g++){
		var gn = (g + 1) % gradSteps.length;
		for (var jump = 0; jump < jumps; jump++){
			grad_output.push(lerpColor(lerpColor(gradSteps[g],gradSteps[gn],jump/jumps),palette.front,0.5));
		}
		// var gn = (g + 1) % gradSteps.length;
		// var glig = gradSteps[g][2] ** 2;
		// var gnlig = gradSteps[gn][2] ** 2;
...
/*
function gradientFactor(huestart, hueend, bright, lightstart, lightend){
	var grad_output = [];
	var huenum = 7;
	var lightnum = 17;
	...
	for (var gradRow = 0; gradRow < lightnum; gradRow++){
		...
		for (var gradCol = 0; gradCol < huenum; gradCol++){
			grad_output[gradRow].push([int((huestart*(huenum - gradCol) + hueend*gradCol)/huenum)%360,bright,int((lightstart*(lightnum - gradRow) + lightend*gradRow)/lightnum)])
		}
	}
